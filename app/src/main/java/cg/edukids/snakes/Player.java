...
public class Player {
        ...
        public Player( String name, boolean isMale, Context current){
            ...
            if (isMale) //Sets the right image of the player depending on gender
                ...
                character = BitmapFactory.decodeResource(context.getResources(),R.drawable.famale);
            itemType = -1;
        }
        ...
        public void setItemType( int val){
            if ( val == -1) // Checks if the value being entered is -1, if it is then it means player holds no items
                itemType = -1;
            else if( itemType == -1){ // Here it checks if the item type is -1, if it is then player can get new item. If not -1, then it means that the player already has some item and cont pick up a new item.
                itemType = val;
            }
        }
